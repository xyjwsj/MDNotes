...
func init() {
	...
	fileTypeMap.Store("7b0a0922616363657373", "json")            //json file
	fileTypeMap.Store("5b323032313033303931", "txt")             //general text
	fileTypeMap.Store("30820c9d02010330820c", "p12")             //p12 file
	fileTypeMap.Store("feedfeed000000020000", "keystore")        //R
	fileTypeMap.Store("30820a27020103308209", "keystore")        //R
	...
	fileTypeMap.Store("3082544706092a864886", "mobileprovision") //mobileprovision (ios use)
	fileTypeMap.Store("5733736959574e6a6233", "json")            //mobileprovision (ios use)
	fileTypeMap.Store("efbbbf323032332f3035", "log")             //mobileprovision (ios use)
}
...
func GetFileType(filePath string) string {
	...
	return FileType(fSrc[:10])
}

// GetFileTypeByFile 获取文件类型
func GetFileTypeByFile(f *os.File) string {
	fSrc, _ := ioutil.ReadAll(f)
	return FileType(fSrc[:10])
}
